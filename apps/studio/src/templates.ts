...
function action(): ProjectDef {
  ...
  s.entities.push(
    E('blob', 'Hero', 360, 640, { color: 0x8affc1 }),
    E('text', 'Hint', 360, 160, { text: 'Move to dodge — TAP enemies to zap!', fontSize: 28, color: 0x9a97b8 }),
  );
  s.script = S([
    '// Arena survival: waves close in on you; tap to zap, touch = down.',
    "api.player('Hero', 340);",
    'var rate = 2.2, foes = [];',
    'function spawnFoe() {',
    '  var side = Math.floor(api.random(0, 4));',
    '  var x = side === 0 ? -40 : side === 1 ? 760 : api.random(0, 720);',
    '  var y = side === 2 ? -40 : side === 3 ? 1320 : api.random(0, 1280);',
    "  var f = api.spawn('blob', x, y);",
    '  f.scale.set(0.75); f.tint = 0xff6f91;',
    '  foes.push(f);',
    '  api.onTap(f, function () { if (!f.destroyed) { api.remove(f); api.sfx.pop(1.4); api.score.add(2); } });',
    '}',
    'var t = 0;',
    'api.onUpdate(function (dt) {',
    '  t += dt;',
    '  if (t > rate) { t = 0; spawnFoe(); rate = Math.max(0.7, rate - 0.05); }',
    "  var hero = api.entity('Hero');",
    '  for (var i = foes.length - 1; i >= 0; i--) {',
    '    var f = foes[i];',
    '    if (f.destroyed) { foes.splice(i, 1); continue; }',
    '    var dx = hero.x - f.x, dy = hero.y - f.y, d = Math.hypot(dx, dy) || 1;',
    '    f.x += (dx / d) * 130 * dt; f.y += (dy / d) * 130 * dt;',
    "    if (d < 55) api.gameOver('OVERRUN! ⚔️');",
    '  }',
    '});',
  ]);
