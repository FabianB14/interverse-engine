...
async function main(): Promise<void> {
  ...
  window.__studio = {
    ...
    saveSlots: () => editor.getPlayScene()?.slots() ?? [],
    slotNow: () => editor.getPlayScene()?.activeSlot ?? 1,
    slotPick: (n: number, fresh: boolean) => editor.getPlayScene()?.pickSlot(n, fresh),
    slotErase: (n: number) => editor.getPlayScene()?.eraseSlot(n),
    levelDone: () => editor.getPlayScene()?.markLevelComplete(),
    unlockedLevels: () => editor.getPlayScene()?.unlockedLevelNames() ?? [],
    /** 🛰 host authority */
    netRole: () => (editor.net ? (editor.net.isHost ? 'host' : 'joiner') : 'solo'),
